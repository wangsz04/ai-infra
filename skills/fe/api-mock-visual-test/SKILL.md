...
description: C 端项目接口场景可视化测试，通过 Playwright MCP 拦截接口、执行页面操作、AI 截屏对比（语义/Figma/基线），从 Swagger 或接口 JSON 自动生成声明式测试场景并执行验证。支持人工审核点和暂停控制。当用户提及 自动化测试/测试项目/模拟接口数据测试/进行E2E测试/进行端到端测试 时应按照该文档进行处理。
---
